...
const http = require('http'); // Para o keep-alive

const app = express();
app.use(express.json());

// SUA API DO BOTFATHER (depois mude para process.env.BOT_TOKEN)
const BOT_TOKEN = '8045598302:AAFD5EN4uelzckSFxp8Wn43na0IdcBolqOo';
const bot = new Telegraf(BOT_TOKEN);

// Lista de usuários permitidos (só quem estiver aqui usa o bot)
const ALLOWED_USER_IDS = [
    7707876089  // Seu ID - Michael
    // Se quiser adicionar mais alguém no futuro: , 123456789
];

// Middleware que bloqueia todo mundo que não está na lista
bot.use((ctx, next) => {
    const userId = ctx.from?.id;

    // Se não tiver ID ou não estiver permitido → ignora silenciosamente
    if (!userId || !ALLOWED_USER_IDS.includes(userId)) {
        console.log(`Acesso negado para userId: ${userId || 'desconhecido'}`);
        return;  // Não responde nada
    }

    // Se for você → continua normalmente
    return next();
});
...
// Keep-alive para plano Free (evita spin down rápido)
setInterval(() => {
  http.get(`http://localhost:${PORT}/`, (res) => {
    console.log('Self-ping: status ' + res.statusCode);
  }).on('error', (err) => {
    console.error('Self-ping erro:', err.message);
  });
