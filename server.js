// ═══════════════════════════════════════════════════════════════════════════════
// NBA BRAIN PRO v10.2 ETERNUM - RESILIENTE (CORRIGIDO)
// Não falha se variáveis estiverem ausentes - usa defaults/mock
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// CONFIGURAÇÃO ETERNUM - COM FALLBACKS
// ═══════════════════════════════════════════════════════════════════════════════

const CONFIG = {
  // FALLBACKS para desenvolvimento/teste
  ...
  // MODO MOCK: true se não tiver database
...
// ═══════════════════════════════════════════════════════════════════════════════
// DATABASE - COM FALLBACK PARA MOCK
// ═══════════════════════════════════════════════════════════════════════════════

let db = null;
let mockStorage = new Map(); // Fallback em memória

async function initDatabase() {
  if (CONFIG.MOCK_MODE) {
    console.log('⚠️  DATABASE_URL não configurado - usando armazenamento em memória (dados serão perdidos no restart)');
    return;
  }

  console.log('🔄 Tentando conectar ao PostgreSQL...');
  console.log(`🔄 URL: ${CONFIG.DATABASE_URL?.substring(0, 50)}...`);
  ...
  try {
    ...
    try {
      ...
      `);
      console.log('💾 Database Eternum inicializado - TABELAS CRIADAS ✅');
    } finally {
      client.release();
    }
  } catch (err) {
...
// Funções de persistência com fallback
async function dbQuery(sql, params) {
  if (CONFIG.MOCK_MODE) {
    // Mock simples - retorna vazio
    return { rows: [] };
  }
  const client = await db.connect();
  try {
...
class SoulEngine {
  ...
  async resurrect() {
    ...
    try {
      ...
      console.log(`✨ [${this.agentId}] RESSURREIÇÃO #${this.incarnation}`);
  ...
  async transmigrate(state) {
    ...
    if (CONFIG.MOCK_MODE) {
      ...
      console.log(`💾 [${this.agentId}] Backup MOCK`);
      return;
    }
...
class TriadCognitive {
  ...
  async deliberate(game, agentProposer) {
    ...
    console.log(`✅ [TRÍADE] ${consensus.decision} | Conf: ${(consensus.confidence*100).toFixed(1)}%`);
    return deliberation;
  }
  ...
  async proposerThink(game, agent) {
    ...
    // Log para debug
    console.log(`🔍 [${agent.id}] Features: ${features.length} elementos | Esperado: ${agent.getInputSize()}`);
    ...
    try {
      ...
      console.error('❌ Erro na predição:', err.message);
      console.error('❌ Features enviadas:', features.length, 'Shape esperado:', agent.getInputSize());
      return {
        decision: 'PASS',
        confidence: 0.5,
...
class ImmortalAgent {
  ...
  extractFeatures(game) {
    // CORRIGIDO: Agora temos 20 features para moneyline
    const features = [
      ...
      // NOVAS FEATURES (9) - Baseadas em estatísticas do agente e do jogo
      (game.homeLast10 || 5) / 20,                    // Win rate home (normalizado diferente)
      (game.awayLast10 || 5) / 20,                    // Win rate away (normalizado diferente)
      ((game.homeRest || 2) + (game.awayRest || 2)) / 10, // Média de descanso
      Math.abs((game.homeRest || 2) - (game.awayRest || 2)) / 5, // Diferença de descanso
      (game.momentumHome || 0) * 0.5,                 // Momentum ajustado
      (game.momentumAway || 0) * 0.5,                 // Momentum ajustado
      this.state.decisions.length / 1000,             // Experiência do agente
      this.state.fitness > 0 ? 1 : 0,                 // Sinal do fitness (positivo/negativo)
      Math.min(1, Math.abs(this.state.fitness) / 50)  // Magnitude do fitness
    ];
    
    // Garantir que temos exatamente o número correto de features
    const targetSize = this.getInputSize();
    while (features.length < targetSize) {
      features.push(0.5); // Preencher com neutros se faltar
    }
...
class EternumEcosystem {
  ...
  selectSpecialist() {
    return Array.from(this.agents.values())
      ...
      .sort((a, b) => b.state.fitness - a.fitness)[0];
  }
...
app.get('/', (req, res) => {
  res.json({
    system: 'NBA BRAIN PRO v10.2 ETERNUM',
    version: '10.2.0',
...
async function main() {
  console.log('\n' + '='.repeat(60));
  console.log('  NBA BRAIN PRO v10.2 ETERNUM - RESILIENTE');
