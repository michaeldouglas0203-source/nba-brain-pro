// ═══════════════════════════════════════════════════════════════════════════════
// NBA BRAIN PRO v10.1 ETERNUM - RENDER FREE COMPATIBLE
// Sem tfjs-node - usa tfjs puro JS
...
// REMOVIDO: require('@tensorflow/tfjs-node'); - Não compatível com Render Free
...
class TriadCognitive {
  ...
  async proposerThink(game, agent) {
    ...
    try {
      ...
      // Fallback para decisão aleatória se NN falhar
      return {
        decision: 'PASS',
        confidence: 0.5,
...
app.get('/', (req, res) => {
  res.json({
    system: 'NBA BRAIN PRO v10.1 ETERNUM',
...
async function main() {
  console.log('\n' + '='.repeat(60));
  console.log('  NBA BRAIN PRO v10.1 ETERNUM');
